...
// Function to perform translation using matrix transformation
...
// Function to perform rotation using matrix transformation
void rotate(std::pair<int, int>* objectCoords, int numCoords, double angle) {
    ...
    for (int i = 0; i < numCoords; ++i) {
        int x = objectCoords[i].first;
        int y = objectCoords[i].second;
        objectCoords[i].first = round(x * cosTheta - y * sinTheta);
        objectCoords[i].second = round(x * sinTheta + y * cosTheta);
    }
}

// Function to perform scaling using matrix transformation
void scale(std::pair<int, int>* objectCoords, int numCoords, double scaleX, double scaleY) {
    for (int i = 0; i < numCoords; ++i) {
        objectCoords[i].first = round(objectCoords[i].first * scaleX);
        objectCoords[i].second = round(objectCoords[i].second * scaleY);
    }
}
